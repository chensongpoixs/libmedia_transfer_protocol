...
//				void* ulpInfo)
//			{
//				auto* sctpAssociation = RtcUsrSctp::RetrieveSctpAssociation(reinterpret_cast<uintptr_t>(ulpInfo));
//
//				if (!sctpAssociation)
//				{
//					//MS_WARN_TAG(sctp, "no SctpAssociation found");
//					LIBRTC_LOG(LS_WARNING) << "sctp: no SctpAssociation found";
//					std::free(data);
//
//					return 0;
//				}
...
//						rcv.rcv_context,
//						flags);*/
//					LIBRTC_LOG(LS_INFO) << "sctp, data chunk received [length:" << len
//						<< ", streamId:" << rcv.rcv_sid << ", SSN:" << rcv.rcv_ssn
//						<< ", TSN:" << rcv.rcv_tsn << ", PPID:" << ntohl(rcv.rcv_ppid)
//						<< ", context:" << rcv.rcv_context << ", flags:" << flags << "] ";
//
...
//			}
//		}
//		SctpAssociation::SctpAssociation(bool isDataChannel)
//			: os(1024u), mis(1024u), maxSctpMessageSize(262144u),
//			sctpSendBufferSize(262144u), isDataChannel(isDataChannel)
//		{
//			// Get a id for this SctpAssociation.
//			this->id = RtcUsrSctp::GetNextSctpAssociationId();
//
...
//				LIBRTC_LOG(LS_ERROR) << "usrsctp_socket() failed:" << std::strerror(errno);
//			}
//
...
//				LIBRTC_LOG(LS_ERROR) << "usrsctp_set_non_blocking() failed:" << std::strerror(errno);
//			}
//
//			// Set SO_LINGER.
...
//				LIBRTC_LOG(LS_ERROR) << "usrsctp_setsockopt(SO_LINGER) failed: " << std::strerror(errno);
//			}
//
...
//			
//				LIBRTC_LOG(LS_ERROR) << "usrsctp_setsockopt(SCTP_ENABLE_STREAM_RESET) failed:" << std::strerror(errno);
//			}
//
//			// Set SCTP_NODELAY.
//			uint32_t noDelay = 1;
...
//				LIBRTC_LOG(LS_ERROR) << "usrsctp_setsockopt(SCTP_NODELAY) failed:" << std::strerror(errno);
//			}
//
//			// Enable events.
...
//					LIBRTC_LOG(LS_ERROR) << "usrsctp_setsockopt(SCTP_EVENT) failed:" << std::strerror(errno);
//				}
//			}
//
//			// Init message.
...
//				LIBRTC_LOG(LS_ERROR) << "usrsctp_setsockopt(SCTP_INITMSG) failed:" << std::strerror(errno);
//			}
//
//			// Server side.
...
//				LIBRTC_LOG(LS_ERROR) << "usrsctp_bind() failed: " << std::strerror(errno);
//			}
//
...
//				LIBRTC_LOG(LS_ERROR) << "usrsctp_setsockopt(SO_SNDBUF) failed:" << std::strerror(errno);
//			}
//
//			// Register the SctpAssociation into the global map.
//			RtcUsrSctp::RegisterSctpAssociation(this);
//		}
...
//			RtcUsrSctp::DeregisterSctpAssociation(this);
//
...
//				{
//					LIBRTC_LOG_T_F(LS_ERROR) << "usrsctp_connect() failed:" << std::strerror(errno);
//				}
...
//				if (ret < 0)
//				{
//					LIBRTC_LOG(LS_ERROR) << "usrsctp_setsockopt(SCTP_PEER_ADDR_PARAMS) failed: " << std::strerror(errno);
//				}
...
//					LIBRTC_LOG(LS_INFO) << "sctp stream reconfiguration not negotiated ";
//					return;
//				}
//			}
//			else
...
//				//	std::strerror(errno));
//				LIBRTC_LOG(LS_WARNING) << "could not retrieve whether stream reconfiguration has been negotiated:" << std::strerror(errno);
//				return;
//			}
//
...
//				LIBRTC_LOG(LS_INFO) << "sctp SCTP_RESET_STREAMS sent [streamId:" << streamId << "]";
//			}
//			else
//			{
//				//MS_WARN_TAG(sctp, "usrsctp_setsockopt(SCTP_RESET_STREAMS) failed: %s", std::strerror(errno));
//			
//				LIBRTC_LOG(LS_WARNING) << "sctp usrsctp_setsockopt(SCTP_RESET_STREAMS) failed: " << std::strerror(errno);
//			}
//
//			std::free(srs);
//		}
...
//				LIBRTC_LOG(LS_WARNING) << "cannot add more outgoing streams [OS:" << os << "]";
//				return;
//			}
//
...
//				LIBRTC_LOG(LS_INFO) << "SCTP not connected, deferring OS increase";
//				return;
//			}
//
...
//			LIBRTC_LOG(LS_INFO) << "adding " << additionalOs << " outgoing streams";
...
//				LIBRTC_LOG(LS_WARNING) << "usrsctp_setsockopt(SCTP_ADD_STREAMS) failed:" << std::strerror(errno);
//			}
//		}
...
//				LIBRTC_LOG(LS_WARNING) << "sctp ignoring SCTP data with ppid:50 (WebRTC DataChannel Control)";
//				return;
//			}
//
...
//				LIBRTC_LOG(LS_WARNING) << "sctp message chunk received with different SSN while buffer not empty, buffer discarded [ssn:"
//					<< ssn << ", last ssn received:" << lastSsnReceived << "]";
...
//				//	eor ? 1 : 0);
//				LIBRTC_LOG(LS_WARNING) << "sctp ongoing received message exceeds max allowed message size [message size:"
//					<< messageBufferLen + len << ", max message size:" << maxSctpMessageSize << ", eor:" << (eor ? 1 : 0);
...
//				LIBRTC_LOG(LS_INFO) << "directly notifying listener [eor:1, buffer len:0]";
...
//				LIBRTC_LOG(LS_INFO) << "notifying listener [eor:1, buffer len:"<< messageBufferLen <<"]";
...
//				LIBRTC_LOG(LS_INFO) << "data buffered [eor:0, buffer len:"<< messageBufferLen <<"]";
//			}
//		}
//		void SctpAssociation::OnUsrSctpReceiveSctpNotification(sctp_notification* notification, size_t len)
//		{
...
//				LIBRTC_LOG(LS_INFO) << "sctp SCTP adaptation indication ["
//					<< notification->sn_adaptation_event.sai_adaptation_ind << "]";
//				break;
//			}
//
...
//					LIBRTC_LOG(LS_INFO) << "sctp SCTP association connected, streams [out:"
//						<< notification->sn_assoc_change.sac_outbound_streams << ", , in:"
//						<< notification->sn_assoc_change.sac_inbound_streams << "]";
//					// Update our OS.
...
//						LIBRTC_LOG(LS_INFO) << "SCTP communication lost [info:" << buffer << "]";
//					}
//					else
//					{
//						//MS_DEBUG_TAG(sctp, "SCTP communication lost");
//						LIBRTC_LOG(LS_INFO) << "SCTP communication lost";
//					}
//
...
//					//	notification->sn_assoc_change.sac_outbound_streams,
//					//	notification->sn_assoc_change.sac_inbound_streams);
//					LIBRTC_LOG(LS_INFO) << "SCTP remote association restarted, streams [out:"
//						<< notification->sn_assoc_change.sac_outbound_streams << ", int:"
//						<< notification->sn_assoc_change.sac_inbound_streams << "]";
//					// Update our OS.
//					this->os = notification->sn_assoc_change.sac_outbound_streams;
//
...
//					LIBRTC_LOG(LS_INFO) << "SCTP association gracefully closed";
//					if (this->state != SctpState::CLOSED)
//					{
//						this->state = SctpState::CLOSED;
//						//this->listener->OnSctpAssociationClosed(this);
...
//						LIBRTC_LOG(LS_INFO) << "SCTP setup failed: " << buffer;
//					}
//
...
//				LIBRTC_LOG(LS_INFO) << "SCTP association reset event received";
//				break;
//			}
//
...
//				//	buffer);
//				LIBRTC_LOG(LS_WARNING) << "remote SCTP association error [type:"
//					<< notification->sn_remote_error.sre_error << ", data:" << buffer << "]";
//				break;
//			}
//
...
//				LIBRTC_LOG(LS_INFO) << "remote SCTP association shutdown";
//				if (this->state != SctpState::CLOSED)
//				{
//					this->state = SctpState::CLOSED;
//					//this->listener->OnSctpAssociationClosed(this);
...
//					buffer);*/
//				LIBRTC_LOG(LS_WARNING) << "SCTP message sent failure [streamId:"
//					<< notification->sn_send_failed_event.ssfe_info.snd_sid
//					<< ", ppid:" << ntohl(notification->sn_send_failed_event.ssfe_info.snd_ppid)
//					<< ", sent:" << ((notification->sn_send_failed_event.ssfe_flags & SCTP_DATA_SENT) ? "yes" : "no")
//					<< ", error:" << notification->sn_send_failed_event.ssfe_error
//					<< ", info:" << buffer;
//				break;
//			}
//
...
//						numStreams,
//						streamIds.c_str());*/
//					LIBRTC_LOG(LS_INFO) << "SCTP stream reset event [flags:" << notification->sn_strreset_event.strreset_flags
//						<< ", , i|o:" << (incoming ? "true" : "false") << "|" << (outgoing ? "true" : "false")
//						<< ", num streams:" << numStreams << ", stream ids:" << streamIds.c_str() << "]";
//				}
//
...
//
//
//					LIBRTC_LOG(LS_INFO) << "SCTP stream changed, streams [out:"
//						<< notification->sn_strchange_event.strchange_outstrms << ", in:"
//						<< notification->sn_strchange_event.strchange_instrms << ", flags:"
//						<< notification->sn_strchange_event.strchange_flags << "]";
//				}
...
//					//	notification->sn_strchange_event.strchange_instrms,
//					//	notification->sn_strchange_event.strchange_flags);
//					LIBRTC_LOG(LS_WARNING) << "SCTP stream change denied, streams [out:"
//						<< notification->sn_strchange_event.strchange_outstrms << ", in:"
//						<< notification->sn_strchange_event.strchange_instrms << ", flags:"
//						<< notification->sn_strchange_event.strchange_flags;
//					break;
...
//					//	notification->sn_strchange_event.strchange_instrms,
//					//	notification->sn_strchange_event.strchange_flags);
//					LIBRTC_LOG(LS_WARNING) << "SCTP stream change failed, streams [out:"
//						<< notification->sn_strchange_event.strchange_outstrms << ", in:"
//						<< notification->sn_strchange_event.strchange_instrms << ", flags:"
//						<< notification->sn_strchange_event.strchange_flags << "]";
//					break;
...
//			{
//				//MS_WARN_TAG(
//				//	sctp, "unhandled SCTP event received [type:%" PRIu16 "]", 
//				//	notification->sn_header.sn_type);
//			
//				LIBRTC_LOG(LS_WARNING) << "unhandled SCTP event received [type:"
//					<< notification->sn_header.sn_type << "]";
//			}
//			}
//		}
