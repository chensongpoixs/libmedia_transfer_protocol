...
******************************************************************************/
#include "libmedia_transfer_protocol/librtcp/transport_sequence_number_feedback.h"
#include <algorithm>
#include <cstdint>
#include <memory>
#include <optional>
...
namespace libmedia_transfer_protocol {
    namespace librtcp {
        ...
        namespace {
            ...
            constexpr webrtc::TimeDelta kBackWindow = webrtc::TimeDelta::Millis(500);
            constexpr webrtc::TimeDelta kMinInterval = webrtc::TimeDelta::Millis(50);
            constexpr webrtc::TimeDelta kMaxInterval = webrtc::TimeDelta::Millis(250);
            constexpr webrtc::TimeDelta kDefaultInterval = webrtc::TimeDelta::Millis(100);

        }  // namespace
#if 1
TransportSequenceNumberFeedback::TransportSequenceNumberFeedback( )
...
void TransportSequenceNumberFeedback::MaybeCullOldPackets(
    int64_t sequence_number,
    int64_t arrival_time) {
  //if (periodic_window_start_seq_ >=
  //        packet_arrival_times_.end_sequence_number() &&
  //    arrival_time - webrtc::Timestamp::Zero() >= kBackWindow) {
  //  // Start new feedback packet, cull old packets.
  //  packet_arrival_times_.RemoveOldPackets(sequence_number,
  //                                         arrival_time.ms_or(0) - kBackWindow.ms_or(0));
  //}
}

void TransportSequenceNumberFeedback::OnReceivedPacket(
...
void TransportSequenceNumberFeedback::Process(int64_t now) {
 // MutexLock lock(&lock_);
  if (!send_periodic_feedback_) {
  ...
  int64_t next_process_time = last_process_time_ + send_interval_;
...
void TransportSequenceNumberFeedback::OnSendBandwidthEstimateChanged(
    webrtc::DataRate estimate) {
  ...
  constexpr webrtc::DataSize kTwccReportSize = webrtc::DataSize::Bytes(20 + 8 + 10 + 30);
  constexpr webrtc::DataRate kMinTwccRate = kTwccReportSize / kMaxInterval;

  // Let TWCC reports occupy 5% of total bandwidth.
  webrtc::DataRate twcc_bitrate = 0.05 * estimate;
  ...
  webrtc::TimeDelta send_interval =
      twcc_bitrate <= kMinTwccRate
          ? kMaxInterval
          : std::max(kTwccReportSize / twcc_bitrate, kMinInterval);

  //MutexLock lock(&lock_);
  send_interval_ = send_interval.ms();
}

void TransportSequenceNumberFeedback::SendPeriodicFeedbacks() {
...
void TransportSequenceNumberFeedback::SendFeedbackOnRequest(
    int64_t sequence_number,
...
TransportSequenceNumberFeedback::MaybeBuildFeedbackPacket(
  ...
  for (int64_t seq = start_seq; seq < end_seq; ++seq) {
    webrtc::PacketArrivalTimeMap::PacketArrivalTime packet =
...
TransportSequenceNumberFeedbackGenenerator(/*RtcpSender feedback_sender*/)
    ...
    last_process_time_(webrtc::Timestamp::MinusInfinity()),
    media_ssrc_(0),
...
#if 0
    if (!packet.GetExtension<libmedia_transfer_protocol::TransportSequenceNumber>(&seqnum) &&
        !packet.GetExtension<TransportSequenceNumberV2>(&seqnum,
            &feedback_request)) {
        // This function expected to be called only for packets that have
        // TransportSequenceNumber rtp header extension, however malformed RTP
...
#endif // 
   // MutexLock lock(&lock_);
    send_periodic_feedback_ = packet.HasExtension<libmedia_transfer_protocol::TransportSequenceNumber>();
...
webrtc::TimeDelta TransportSequenceNumberFeedbackGenenerator::Process(webrtc::Timestamp now) {
   // MutexLock lock(&lock_);
    if (!send_periodic_feedback_) {
        // If TransportSequenceNumberV2 has been received in one packet,
        // PeriodicFeedback is disabled for the rest of the call.
    ...
    webrtc::Timestamp next_process_time = last_process_time_ + send_interval_;
    if (now >= next_process_time) {
        last_process_time_ = now;
        SendPeriodicFeedbacks();
