...
StatusVecChunk::StatusVecChunk(bool symbol_bit, const std::vector<SymbolStatus> &status) {
    RTC_CHECK(status.size() << symbol_bit <= 14);
    uint16_t value = 0;
    type = 1;
    symbol = symbol_bit;
    int i = 13;
    for (auto &item : status) {
        RTC_CHECK(item <= SymbolStatus::reserved);
        if (!symbol) {
            RTC_CHECK(item <= SymbolStatus::small_delta);
            value |= (int)item << i;
            --i;
        } else {
            value |= (int)item << (i - 1);
...
FCI_TWCC::TwccPacketStatus FCI_TWCC::getPacketChunkList(size_t total_size) const {
    ...
    for (uint16_t i = 0; i < rtp_count;) {
        RTC_CHECK(ptr + RunLengthChunk::kSize <= end);
...
std::string FCI_TWCC::create(uint32_t ref_time, uint8_t fb_pkt_count, TwccPacketStatus &status) {
    ...
    while (!status.empty()) {
...

        {
            ...
            vec.resize(std::min(vec.size(), (size_t)14 >> symbol));
