...
namespace libmedia_transfer_protocol {
    namespace librtcp {
        ...
        namespace {
         ...
         static  std::string LogChannelPrintTime(const    timeval& tv) {
...
std::string RtcpHeader::dumpString() const {
    switch ((RtcpType)pt) {
        ...
        default: return StrPrinter << dumpHeader() << rtc::hex_encode((char *)this + sizeof(*this), getSize() - sizeof(*this));
    }
}
    ...
    switch ((RtcpType)pt) {
        ...
        case RtcpType::RTCP_XR: {
            ...
            if (xr->bt == 4) {
                ...
                dlrr->net2Host(len);
                LIBRTC_LOG(LS_INFO) << dlrr->dumpString();
            } else if (xr->bt == 42){
...
std::vector<RtcpHeader *> RtcpHeader::loadFromBytes(char *data, size_t len) {
    ...
    int32_t remain = len;
    char *ptr = data;
    while (remain > (int32_t)sizeof(RtcpHeader)) {
        RtcpHeader *rtcp = (RtcpHeader *)ptr;
        auto rtcp_len = rtcp->getSize();
        if (remain < (int32_t)rtcp_len) {
            LIBRTC_LOG_F(LS_WARNING) << "非法的rtcp包,声明的长度超过实际数据长度";
            break;
        }
        try {
            ...
            LIBRTC_LOG_F(LS_WARNING) << ex.what() << ",长度为:" << rtcp_len;
        }
        ptr += rtcp_len;
        remain -= rtcp_len;
    }
...
#define CHECK_REPORT_COUNT(item_count)                                                                                 \
    ...
    if (report_count != item_count) {                                                                                  \
        LIBRTC_LOG(LS_WARNING) << rtcpTypeToStr((RtcpType)pt) << " report_count 字段不正确,已修正为:" << (int)report_count << " -> "    \
              << item_count;                                                                                           \
...
size_t RtcpFB::getFciSize() const {
    auto fci_len = (int64_t)getSize() - getPaddingSize() - sizeof(RtcpFB);
    RTC_CHECK(getSize() >= getPaddingSize() + sizeof(RtcpFB));
    return fci_len;
}
        ...
        case RtcpType::RTCP_PSFB: {
            switch ((PSFBType)report_count) {
                case PSFBType::RTCP_PSFB_SLI: {
                    auto &fci = getFci<::libmedia_transfer_protocol::librtcp::FCI_SLI>();
                ...
                default: {
                    printer << "fci:" << psfbTypeToStr((PSFBType)report_count) << " "
                            << rtc::hex_encode((const char *)getFciPtr(), getFciSize());
                    break;
                }
            }
            break;
...
void RtcpBye::net2Host(size_t size) {
    ...
    if (offset < size) {
        ...
        if (reason_len_ptr + 1 + *reason_len_ptr > (uint8_t *)this + size) {
            LIBRTCP_LOG_F(LS_WARNING) << "invalid rtcp bye reason length";
...
////////////////////////////////////
std::string RtcpXRTargetBitrateItem::dumpString() const {
    _StrPrinter printer;
...
std::vector<RtcpXRTargetBitrateItem *> RtcpXRTargetBitrate::getItemList() {
    ...
    std::vector<RtcpXRTargetBitrateItem *> ret;
    for (int i = 0; i < (int)count; ++i) {
        ret.emplace_back(ptr);
        ++ptr;
    }
...
std::string RtcpXRTargetBitrate::dumpString() const {
    _StrPrinter printer;
    printer << RtcpHeader::dumpHeader();
    printer << "ssrc :" << ssrc << "\r\n";
    printer << "bt :" << (int)bt << "\r\n";
