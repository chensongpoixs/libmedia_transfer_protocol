...
/***********************************************************************************************
 ...
 purpose:		AMF0 Format - AMF0格式编解码器实现
 输赢不重要，答案对你们有什么意义才重要。
...
*  
*  用于检测系统的字节序（大端序或小端序）。
*  如果s_double的第一个字节为0x00，则为小端序；否则为大端序。
*/
...
static uint8_t* AMFWriteInt32(uint8_t* ptr, const uint8_t* end, uint32_t value)
{
	...
	ptr[0] = (uint8_t)(value >> 24);  // 最高8位
	ptr[1] = (uint8_t)(value >> 16);  // 次高8位
	ptr[2] = (uint8_t)(value >> 8);   // 次低8位
	ptr[3] = (uint8_t)(value & 0xFF); // 最低8位
	return ptr + 4;
}
...
/**
*  @brief 写入AMF0 Double值实现
*  
*  写入AMF_NUMBER类型标记和8字节双精度浮点数。
*  浮点数使用大端序存储，需要根据系统字节序进行转换。
*  
*  格式：[类型标记(1字节)][双精度浮点数(8字节，大端序)]
*/
...
/**
*  @brief 写入AMF0 String值实现
*  
*  根据字符串长度选择使用短字符串（2字节长度）或长字符串（4字节长度）格式。
*  
*  短字符串格式：[AMF_STRING(1字节)][长度(2字节)][字符串内容]
*  长字符串格式：[AMF_LONG_STRING(1字节)][长度(4字节)][字符串内容]
*/
...
/**
*  @brief 读取AMF0 Double值实现
*  
*  从内存中读取8字节双精度浮点数，并根据系统字节序进行转换。
*  
*  @param ptr 源内存地址
*  @param end 缓冲区结束位置
...
/**
*  @brief 读取AMF0 String值实现
*  
*  根据isLongString参数读取短字符串或长字符串。
*  
*  @param ptr 源内存地址
*  @param end 缓冲区结束位置
...
{ 
	...
	// 读取字符串长度
	if (0 == isLongString)
		ptr = AMFReadInt16(ptr, end, &len);  // 短字符串（2字节长度）
	else
		ptr = AMFReadInt32(ptr, end, &len);  // 长字符串（4字节长度）
